...
def manage_user_role(request, userprofile_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Admin':
			data = UserProfile.objects.get(pk=userprofile_id)
			form = UserProfileForm(request.POST or None, instance=data)
			if form.is_valid():
				form.save()
				messages.add_message(request, messages.SUCCESS, 'User Role Updated')
...
def update_user(request, user_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Admin':
			...
			userprof = UserProfile.objects.filter(user__pk=user_id)
			userprof=userprof[0].id
			form = CreateUserForm(request.POST or None, instance=USER)
			if form.is_valid():
				form.save()
				messages.add_message(request, messages.SUCCESS, 'User Information Updated in the Database')
...
def delete_user(request, user_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Admin':
			...
			if USER:
				USER.delete()
				messages.add_message(request, messages.SUCCESS, 'User Removed From the Database')
	...
	if request.user.is_authenticated:
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			...
			if request.method == 'POST':
				...
				if form.is_valid():
					form.save()
					messages.add_message(request, messages.SUCCESS, 'Owner Registered in the Database')
					return redirect('display-database')
				else:
					messages.add_message(request, messages.INFO, 'Encoutered Error while Registering Owner')
	...
	if request.user.is_authenticated:
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			...
			if request.method == 'POST':
				...
				if form.is_valid():
					form.save()
					messages.add_message(request, messages.SUCCESS, 'Registered a vehicle on The Following Owner')
					return redirect('display-database')
		...
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			...
			if request.method == 'POST':
				...
				if ownerForm.is_valid() & vehicleForm.is_valid():
					ownerForm.save()
					vehicleForm.save()
					messages.add_message(request, messages.SUCCESS, 'Data Stored')
					return redirect('display-database')
				else:
					messages.add_message(request, messages.INFO, 'Error Found While Storing Data')
					return redirect('display-database')
...
def update_owner(request, owner_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			vehicle = vehicle_license_plate_registration_table.objects.filter(registered_owner_id__pk=owner_id)
			indiv = registered_vehicle_owner_table.objects.get(pk=owner_id)
			form = VehicleOwnerForm(request.POST or None, instance=indiv)
			if form.is_valid():
				form.save()
				messages.add_message(request, messages.SUCCESS, 'Vehicle Owner Information Updated in the Database')
...
def update_vehicle(request, vin):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			...
			if form.is_valid():
				form.save()
				messages.add_message(request, messages.SUCCESS, 'Vehicle Registration Information Updated in the Database')
...
def generate_owner_details(request, owner_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'BRTA_Staff' or checkuserrole(request) == 'Admin':
			...
			lines.append(f"Detailed Report on {owner.registered_vehicle_owner}")
			lines.append('_________________________________________________________________________________________')
			lines.append("Registered Owner's details")
			lines.append('_________________________________________________________________________________________')
			lines.append(f'Registered Vehicle Owner: {owner.registered_vehicle_owner}')
			lines.append(f'registered Vehicle Owner ID: {owner.registered_owner_id}')
			lines.append(f"Registered Owner's Address: {owner.registered_owner_address}")
			lines.append(f"Registered Vehicle Owner's DOB: {owner.registered_owner_dob}")
			lines.append(f"Registration Date: {owner.registered_owner_data_create}")
			lines.append('')
			lines.append('_________________________________________________________________________________________')
			lines.append(f"Registered Owned Vehicle(s)")
			lines.append('_________________________________________________________________________________________')
			
			vehicles = vehicle_license_plate_registration_table.objects.filter(registered_owner_id__pk=owner_id)

			if vehicles:
				for vehicle in vehicles:
					lines.append(f"City Name: {vehicle.city_name}")
					lines.append(f"Vehicle Classification: {vehicle.vehicle_classification}")
					...
					lines.append(f"Vehicle Registered To: {vehicle.registered_owner_id.registered_vehicle_owner}")
					lines.append('')
				lines.append('_________________________________________________________________________________________')
			else:
				lines.append(f"{owner.registered_vehicle_owner} has no Registered Vehicle(s)")
				lines.append('_________________________________________________________________________________________')
	...
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			...
			if request.method == 'POST':
				...
				if form.is_valid():
					form.save()
					messages.add_message(request, messages.SUCCESS, 'Incident Registered Successfully')
					vehicle = vehicle_license_plate_registration_table.objects.get(pk=vin)
					felonys = IncidentVehicular.objects.filter(vin__pk=vin)
...
def run_vin(request):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			if request.method == 'POST':
				try:
					vehicle = vehicle_license_plate_registration_table.objects.get(pk=request.POST['searched'])
					felonys = IncidentVehicular.objects.filter(vin__pk=request.POST['searched'])
				except:
					vehicle= None
					felonys = None
					pass
...
def display_owner(request, owner_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			...
			indiv = registered_vehicle_owner_table.objects.get(pk=owner_id)
			form = VehicleOwnerForm(request.POST or None, instance=indiv)
			felonys = IncidentVehicular.objects.filter(registered_owner_id__pk=owner_id)
...
def generate_incident(request, incident_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			...
			lines.append('')
			lines.append(f'Registered Vehicle Owner: {incident.registered_owner_id.registered_vehicle_owner}')
			lines.append('')
			lines.append(f'Registered Vehicle OwnerID: {incident.registered_owner_id.registered_owner_id}')
			lines.append('')
...
def generate_bill(request, incident_id):
	if request.user.is_authenticated:
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			...
			lines.append(f'Vehicle Identification Number: {incident.vin}')
			lines.append('')
			lines.append(f'Registered Vehicle Owner ID: {incident.registered_owner_id.registered_owner_id}')
			lines.append('')
			lines.append(f'Registered Vehicle Owner: {incident.registered_owner_id.registered_vehicle_owner}')
			lines.append('')
			felFine = 0
			...
			lines.append('')
			lines.append(f'Please pay the Net Payable Ammount: {felFine} Taka to any branch of Sonali Bank')
			lines.append('_________________________________________________________________________________________')
			lines.append('')
		...
		if checkuserrole(request) == 'Police' or checkuserrole(request) == 'Admin':
			...
			if vehicles:
				for vehicle in vehicles:
					...
					lines.append('')
				lines.append('_________________________________________________________________________________________')
			lines.append(f"Felonies {owner.registered_vehicle_owner} has been committed")

			felonies = IncidentVehicular.objects.filter(registered_owner_id__pk=owner_id)
			lines.append('_________________________________________________________________________________________')
			if felonies:
				for felony in felonies:
					...
					lines.append('')
			if felonies == None:
				lines.append(f"{owner.registered_vehicle_owner} has never been committed any felony(ies)")
			lines.append('_________________________________________________________________________________________')
			lines.append(f"Printed by {request.user}")
			# Putting lines in text object
			for line in lines:
